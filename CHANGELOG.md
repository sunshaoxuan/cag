...
* 增加 MIME、魔数、容器结构和文本概率联合探测，未知扩展名的文本资料可以进入清洗流程。
* 增加显式提取器注册表和无网络隔离 Worker，父进程执行硬超时与进程终止。
* 增加输入、输出、归档成员、展开量、压缩比、路径穿越和链接限制，输入文件不会被执行。
* 增加 RTF、EML、MSG、旧 XLS、OLE 文本流、安全 ZIP 和常见图片 OCR 处理器。
* Source Entry 和失败审计记录处理器、版本、稳定原因码、可重试性、检测 MIME、魔数和文本概率。
* 清洗后的完整文本按 SHA 256 写入双副本 Artifact，并与 Source Entry 建立可反查位置。
...
* 新增内容寻址 `Artifact`、来源观察位置、对象副本、转换和完整性对账运行模型，全部使用独立物理 UUID 和数据库外键。
* 新增 `ArtifactObjectStore` 接口、双目录独立副本实现和 S3 兼容适配器。RustFS 通过标准 S3 配置接入，不向业务代码暴露专用 API。
* 新增 Artifact 摘要、按 SHA 256 查询、完整内容读取和完整性对账 API。
* 企业知识管理页显示内容寻址证据对象和健康副本数量。

### Changed

* Phase 1 对象写入采用上传、读取校验、双副本完成、数据库发布的顺序。数据库只发布具备两个已校验副本的 Artifact。
* 原始路径继续作为可变化的来源观察。完整清洗对象由 SHA 256 对象键持久保存，路径断开后可以从健康副本读取。
* RustFS 分布式模式通过项目定义的耐久性门以前，正式运行保留独立第二副本。

### Safety

* 对象键和内容必须同时匹配 SHA 256。内容不符、键不符和存储损坏均拒绝发布。
* S3 凭据只通过 Secret 配置进入客户端，不写入数据库、日志或 API。Artifact 使用独立 Credential Manager 密钥，数据库只保存非秘密 Key ID。
* 普通读取只接受数据库状态为 `available` 的 Artifact。所有副本不可用时返回稳定的 `artifact_unavailable`。
* 对账识别存储孤立对象，孤立对象不会进入普通检索。

### Validated

* 自动化测试覆盖重复 Hash 去重、双副本写入、原路径断开读取、主副任一方向恢复、不可用状态、孤立对象统计和 API 往返。
* Alembic 升级、降级和发布版本一致性由迁移与版本门验证。
...
* 新增 CAG 证据与经验系统演进路线图及 ADR 0032，规划内容寻址对象证据、通用安全文本抽取、现有知识转换与原子重建、证据经验作业关系网格、批准经验注入与图引擎基准选型。所有新增能力在实现和验收前保持 `Planned`。

### Changed

* Windows 监督任务新增每分钟重复的看门狗触发器。系统启动、用户登录和周期触发共同覆盖主机休眠、会话中断及监督进程异常结束后的恢复。

### Fixed

* 监督任务随主机会话异常结束后，启动与登录触发器均不会再次发生，导致 Gateway 长时间离线的问题。
* Docker Desktop 恢复后 PostgreSQL 与 Redis 主机端口转发可能失效，容器内部健康而主机连接超时的问题纳入运行验收与恢复记录。
* API 测试台在 LAN HTTP 页面调用仅限安全上下文的 `crypto.randomUUID()`，导致任务请求在浏览器端中断的问题。请求 ID 和幂等键现统一由 `crypto.getRandomValues()` 生成 RFC 4122 v4 UUID。
* 大规模知识库的词法检索把 Chunk 正文、Document 路径和 Source 子路径放入跨表 OR，导致 PostgreSQL 无法形成稳定的有界索引计划并触发 statement timeout。三个召回通道现分别执行有界查询，再进入统一融合排序。
* 中文简体客户名称无法命中日文新字体路径，以及单个代码文档的多个 Chunk 占满结果页的问题。查询现生成简体、繁体和日文新字体变体，并对最终结果执行每文档最多两个优先 Chunk 的多样化排序。
* `old`、`旧`、`back`、`backup`、`bak`、`バックアップ` 和 `#history` 资料仍可进入普通知识搜索的问题。历史路径判断现由客户台账抽取和所有搜索候选通道共同使用。
* PowerShell 运行测试遗留 8001、8002 临时监督任务和共享 Redis Worker，导致正式任务可能被旧代码进程领取的问题。测试入口现清理非正式 CAG 监督任务，并验证最终只保留正式 8000 任务。
