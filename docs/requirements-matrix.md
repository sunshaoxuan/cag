...
| Scoped customer ledger knowledge extraction | Implemented | request schema v1 with analysis Template v2, dedicated extraction queue, lease-based liveness, public per-document progress, HTTP-enforced per-file model deadline, Catalog Scope resolution, exhaustive manifest, file checkpoints, typed fields, coverage, conflicts, unresolved fields and stable errors |
