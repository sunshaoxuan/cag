...
当前版本为 `0.19.0`。Gateway 默认监听全部 IPv4 网络接口。Windows 监督任务在系统启动和用户登录时自动运行，持续检查健康状态，并在连续异常或进程退出后延迟恢复。正式运行时使用 PostgreSQL 16 加 pgvector，向量召回由数据库内余弦距离算子和 HNSW 索引执行。PostgreSQL 同时保存交互任务和知识学习队列，Redis 负责跨进程即时唤醒，工作器通过租约、心跳、有限重试和启动恢复接续任务。5173 端口提供统一的 CAG 可视化管理台，包含 API 测试、调用监控、在线 API 文档、企业知识、代码知识、长期记忆和能力治理。API 测试台将调查、执行和复核 Agent 的中间回答收纳在默认折叠的灰色区域，Task 进入终态后单独显示支持 GitHub 风格 Markdown 的最终报告。企业知识来源保留为持久注册表和文件资产清单，可按计划重复扫描。ZIP、DUMP、备份、二进制及超限大文件只登记存在性和路径元数据，零字节文件以路径知识入库，源代码固定进入结构化分析。处理策略指纹允许工具升级后重新处理未改动文件，符合当前策略的未变化文档继续复用向量。每轮同步保存历史、租约、重试、来源状态和逐文件结果。每个被拒绝或跳过的文件保存相对路径、原因和异常证据，支持管理画面查询、CSV 导出以及自动生成并定期清理的 gzip 归档。代码索引保存结构化符号、调用与依赖关系、文档关联证据、解析器和编码元数据。文件夹来源使用逐目录队列扫描，每次只打开一个目录，并通过 SSE 持续反馈当前相对目录、已扫描目录、待扫描目录和文件进度。每轮 Conversation 会先检索已批准知识，再把片段、原始资源 URI、路径和 commit 交给本机 Codex app-server 分析，最终报告和记忆候选保留同一引用链。编辑已配置来源时可以按需读取 Windows 凭据库中的密码或令牌，并在当前管理页面显示或复制。管理台通过同源反向代理连接本机订阅认证的 Codex Gateway。外部系统可以通过任务 API 和持续会话 API 调用 CAG。每次任务返回 Trace ID，支持幂等提交，并将知识、Harness、Agent、命令、审批、验证和学习动作写入统一审计 SSE。
