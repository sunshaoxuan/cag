...
当前版本为 `0.22.6`。Gateway 监听全部 IPv4 网络接口，由 Windows 监督任务持续检查健康状态。正式运行时使用 PostgreSQL 16、pgvector 和 Redis，分别承载业务真相、向量召回和跨进程唤醒。交互任务、知识学习和自运维问题使用独立 Worker 池，并通过租约、心跳、有限重试和启动恢复接续工作。问题中心的审批和状态变更要求管理员令牌与身份头，AI 逐字增量不会进入长期问题时间线。
